...
Process job-offer URLs accumulated in `data/pipeline.md`. The user adds URLs whenever they want and then runs `/career-ops pipeline` to process them all.
...
**Special cases:**
- **LinkedIn**: May require login → mark as `[!]` and ask the user to paste the text
